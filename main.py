            ...
            while True:
                try:
                    ...
                    if record_url.find("https://live.douyin.com/") > -1:
                        ...
                        with semaphore:
                            json_data = get_douyin_stream_data(
                                new_record_url, cookies=dy_cookie)
                            port_info = get_douyin_stream_url(
                                json_data, record_quality)
                        ...
                        with semaphore:
                            json_data = get_bilibili_stream_data(
                                record_url, cookies=bili_cookie)
                        ...
                        with semaphore:
                            port_info = get_qiandurebo_stream_data(
                                url=record_url, cookies=qiandurebo_cookie)
                        ...
                        with semaphore:
                            port_info = get_maoerfm_stream_url(
                                url=record_url, cookies=maoerfm_cookie)
                                    ...
                                    if split_video_by_time:
                                        ...
                                        if create_time_file:
                                            filename_gruop = [
                                                anchor_name, filename_short]
                                            create_var[str(filename_short)] = threading.Thread(
                                                target=create_ass_file, args=(filename_gruop,))
                                            create_var[str(
                                                filename_short)].daemon = True
                                            create_var[str(
                                                filename_short)].start()
...
while True:
    ...
    video_save_type = read_config_value(
        config, '录制设置', '视频保存格式TS|MKV|FLV|MP4|TS音频|MKV音频', "mp4")
    ...
    feishu_webhook_url = read_config_value(
        config, '推送配置', '飞书Webhook机器人Url', "")
    feishu_webhook_secret = read_config_value(
        config, '推送配置', '飞书Webhook机器人Secret', "")
